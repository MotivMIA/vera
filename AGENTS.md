...
1. Two files: **`.env.local`** (local dev) and **`.env.production.local`** (live). Run `npm run env:setup` from `.env` or copy the `*.example` templates.
2. **`npm run dev`** loads `.env.local` only — do not rely on a lone `.env` for dev.
