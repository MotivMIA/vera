...
1. Copy **`.env.local.example` → `.env.local`** (gitignored). Or `npm run env:migrate` from `.env.dev`, or `npm run env:split` from `.env`.
2. **`npm run dev`** — Next.js loads `.env.local` automatically. Set `NEXT_PUBLIC_SITE_URL=http://localhost:3001`.
