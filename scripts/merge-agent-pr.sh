#!/usr/bin/env bash
# Merge an agent PR after interactive Enter confirmation.
...
if [[ "$MERGE_STATE" != "CLEAN" && "$MERGE_STATE" != "UNSTABLE" ]]; then
  ...
  echo "Common causes: unresolved review threads, missing approvals, branch behind main." >&2
  exit 1
fi

echo "All checks are green."
echo ""
echo "Press Enter to merge this PR, or Ctrl+C to cancel."
read -r
